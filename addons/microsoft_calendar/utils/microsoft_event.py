...
class MicrosoftEvent(abc.Set):
    ...
    Inspired by midrarsolutions recordset, one instance can be a single Microsoft event or a
    ...
    def _meta_odoo_id(self, microsoft_guid):
        """Returns the midrarsolutions id stored in the Microsoft Event metadata.
    ...
    def _load_odoo_ids_from_metadata(self, env):
        ...
        # Hence, we may have two Microsoft recurrences linked to the same midrarsolutions id.
        # Therefore, we only consider midrarsolutions records without microsoft id when trying
        # to match events.
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Microsoft and midrarsolutions.
        # Let userA, userB be two new users (never synced to Microsoft before).
        # UserA creates an event in midrarsolutions (he is the owner) but userB syncs first.
        ...
        # (he is the orginizer in Microsoft). The "real" owner (in midrarsolutions) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Microsoft: 1 email = 1 user; but in midrarsolutions several users might have the same email
