...
@tagged("odoobot")
class Testmidrarsolutionsbot(TestMailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(Testmidrarsolutionsbot, cls).setUpClass()
        ...
        cls.odoobot_ping_body = '<a href="http://midrarsolutions.app/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@midrarsolutionsBot</a>' % (cls.odoobot.id, cls.odoobot.id)
    ...
    def test_odoobot_ping(self):
        ...
        # midrarsolutionsbot should not be a follower but user_employee and user_admin should
