...
class Meeting(models.Model):
    ...
    def _google_values(self):
        ...
        if self.user_id and self.user_id != self.env.user and not bool(self.user_id.sudo().google_calendar_token):
            # The organizer is an midrarsolutions user that do not sync his calendar
            ...
            # the owner is not an midrarsolutions user: We don't store the real owner identity (mail)
