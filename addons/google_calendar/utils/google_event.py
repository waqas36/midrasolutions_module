...
class GoogleEvent(abc.Set):
    ...
    Inspired by midrarsolutions recordset, one instance can be a single Google event or a
    ...
    def _meta_odoo_id(self, dbname):
        """Returns the midrarsolutions id stored in the Google Event metadata.
    ...
    def _load_odoo_ids_from_metadata(self, env, model):
        ...
        # Hence, we may have two Google recurrences linked to the same midrarsolutions id.
        # Therefore, we only consider midrarsolutions records without google id when trying
        # to match events.
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and midrarsolutions.
        ...
        # (he is the organizer in Google). The "real" owner (in midrarsolutions) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in midrarsolutions several users might have the same email :/
