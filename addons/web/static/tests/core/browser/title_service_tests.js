...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "midrarsolutions" });
    assert.strictEqual(env.services.title.current, "midrarsolutions");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "midrarsolutions", chat: null });
    assert.strictEqual(env.services.title.current, "midrarsolutions");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "midrarsolutions - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "midrarsolutions", action: "Import" });
    assert.strictEqual(env.services.title.current, "midrarsolutions - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "midrarsolutions", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "midrarsolutions - Import"); // parts is a copy!
});
