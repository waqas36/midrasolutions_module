...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test("flags field of ir.actions.act_window is used", async function (assert) {
        // more info about flags field : https://github.com/odoo/midrarsolutions.appmit/c9b133813b250e89f1f61816b0eabfb9bee2009d
        assert.expect(7);
    ...
    QUnit.test("pushState also changes the title of the tab", async (assert) => {
        ...
        assert.strictEqual(titleService.current, '{"zopenerp":"midrarsolutions","action":"Partners"}');
        await click(webClient.el.querySelector(".o_data_row"));
        await legacyExtraNextTick();
        assert.strictEqual(titleService.current, '{"zopenerp":"midrarsolutions","action":"First record"}');
        await click(webClient.el.querySelector(".o_pager_next"));
        assert.strictEqual(titleService.current, '{"zopenerp":"midrarsolutions","action":"Second record"}');
    });
