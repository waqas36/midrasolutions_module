 ...
 * not fire.
 *
 * @param {Widget} widget the target widget (any midrarsolutions widget)
...
async function addMockEnvironmentOwl(Component, params, mockServer) {
    ...
    function cleanUp() {
        ...
        $('.blockUI').remove(); // fixme: move to qunit_config in midrarsolutionsAfterTestHook?
 ...
 * @param {number} [params.debug] if set to true, logs RPCs and uncaught midrarsolutions
 *   events.
...
async function addMockEnvironment(widget, params) {
    ...
    // make sure all other midrarsolutions events bubbling up are intercepted
